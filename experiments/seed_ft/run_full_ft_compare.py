...
def evaluate(model, dataloader, criterion, device):
    ...
    # SEED has 3 classes
    f1 = f1_score(all_targets, all_preds, average='weighted', labels=list(range(3)), zero_division=0)
    ...
    try:
        # Binarize targets for AUC-PR
        all_targets_bin = label_binarize(all_targets, classes=[0, 1, 2])
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/finetune_seed.yaml')
    ...
    parser.add_argument('--output_dir', type=str, default='experiments/seed_ft/results')
    ...
    # Load all files
    # get_seed_file_list normally does a split, but here we want all files first to shuffle and split manually
    # However, get_seed_file_list returns sorted list if we ask for 'train' or 'test' based on its logic.
    # But it reads directory content.
    # Let's verify SEED dataset size: 15 subjects.
    
    # We'll use a trick: call with 'train' to get all files if we pass a huge split, 
    # OR better, modify how we get files.
    # Actually get_seed_file_list returns a subset.
    # Let's just list files directly or use the function if it exposes all.
    # Looking at seed.py: it lists all files, sorts them, then splits.
    # We can reconstruct "all_files" by calling it for train/test/val if they cover everything, 
    # OR just list dir here.
    ...
    if total_subjects != 15:
        print(f"Warning: Expected 15 subjects, found {total_subjects}")
    ...
    base_config['model']['num_classes'] = 3 # SEED: Positive, Neutral, Negative
    base_config['model']['seq_len'] = 2 # 400/200
    ...
    for ratio in sorted(args.ratios):
        ...
        # Split: 12 Train, 3 Test (for 100%)
        # If ratio < 1.0, we scale down.
        # But user requirement is "12 train, 3 test".
        # If ratio is used, we assume it's for the training set size? 
        # Or just subsetting the whole pool?
        # Usually ratio subsets the *available* subjects.
        
        # Standard full SEED split: 12 train, 3 test.
        # If n_subs < 15, we adjust.
        # Let's try to maintain 12/3 proportion or just take 3 for test if possible.
        
        n_test = 3
        n_train = n_subs - n_test
        if n_train <= 0:
            n_train = max(1, int(n_subs * 0.8))
        ...
        # Initialize Datasets
        # Note: SEEDDataset takes mode argument but it mainly affects cache path and cropping
        train_dataset = SEEDDataset(train_files_subset, mode='train')
        val_dataset = SEEDDataset(val_files_subset, mode='test') if val_files_subset else None
        test_dataset = SEEDDataset(test_files_subset, mode='test') if test_files_subset else None
    ...
    with open(report_path, 'w') as f:
        f.write("# SEED Full Fine-tuning Comparative Results (12 Train / 3 Test)\n\n")
