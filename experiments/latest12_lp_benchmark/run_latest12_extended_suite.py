#!/usr/bin/env python3
"""Extended linear-probing benchmark for latest-12 checkpoints.

This script is identical in structure to run_latest12_suite.py but adds
the downstream datasets that were missing from the original run:
  SleepEDF_full, Physionet_MI, Workload, MDD, AD65

All 12 datasets covered:
  BCIC2A, SEEDIV, TUAB, TUEP, TUEV, TUSZ,
...
# ---------------------------------------------------------------------------
# Best checkpoints (same as run_latest12_suite.py)
# ---------------------------------------------------------------------------
...
ALL_DATASETS = (
    "TUAB,BCIC2A,SEEDIV,TUEP,TUEV,TUSZ,"
    "SleepEDF_full,Physionet_MI,Workload,SEED,MDD,AD65"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extended LP benchmark: 12 checkpoints x 12 datasets."
    )
    parser.add_argument(
        "--benchmark_script",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/latest12_lp_benchmark/results_extended",
...
# Aggregate
# ---------------------------------------------------------------------------
def aggregate_results(records: List[Dict], output_root: Path) -> None:
    long_rows = []
    status_rows = []
    ...
    for rec in records:
        ...
        if not required.issubset(set(df.columns)):
            print(f"[WARN] Missing columns in {per_csv}: {required - set(df.columns)}")
            continue
    ...
    # Pivot: Dataset x Model -> BAcc
    bacc_pivot = long_df.pivot_table(
        index="Dataset", columns="Model_Tag",
        values="Balanced_Accuracy", aggfunc="first"
    )
    bacc_pivot.to_csv(output_root / "results_compare_bacc.csv")
    print(f"[OK] Saved BAcc comparison: {output_root / 'results_compare_bacc.csv'}")

    acc_pivot = long_df.pivot_table(
        index="Dataset", columns="Model_Tag",
        values="Accuracy", aggfunc="first"
    )
    acc_pivot.to_csv(output_root / "results_compare_acc.csv")
    print(f"[OK] Saved Acc comparison: {output_root / 'results_compare_acc.csv'}")

    # Ranking by mean BAcc
    summary = (
    ...
    meta_df = long_df[
        ["Model_Tag", "Run_Dir", "Model_Name", "Model_Family", "Checkpoint"]
    ].drop_duplicates()
    summary = summary.merge(meta_df, on="Model_Tag", how="left")
    summary.to_csv(output_root / "results_rank_mean_bacc.csv", index=False)
    print(f"[OK] Saved ranking: {output_root / 'results_rank_mean_bacc.csv'}")
...
def main() -> None:
    ...
    print(f"[INFO] {len(checkpoints)} checkpoints discovered.")
    for rec in checkpoints:
        print(f"[INFO]   {rec['model_tag']} -> {rec['checkpoint']}")

    if not checkpoints:
        raise RuntimeError("No checkpoints found. Check BEST_CHECKPOINT_PATHS.")

    for rec in checkpoints:
        safe_name = rec["model_tag"].replace("/", "__")
        per_model_csv = per_model_dir / f"{safe_name}.csv"
        rec["per_model_csv"] = str(per_model_csv)

        if args.skip_existing and per_model_csv.is_file():
            print(f"[SKIP] {per_model_csv}")
            rec["return_code"] = 0
            continue
        ...
        if ret != 0:
            print(f"[FAIL] {rec['model_tag']} (code {ret})")
            if args.stop_on_error:
                break
        else:
            print(f"[OK] {rec['model_tag']}")

    aggregate_results(checkpoints, output_root)
    print("[DONE] Extended LP benchmark finished.")
