...
# Paths
OUTPUT_DIR="experiments/bcic2a_full_ft/results_flagship"
CONFIG="configs/finetune_bcic2a.yaml"
...
REQUIRED_FILES=("$CONFIG" "$FLAGSHIP_WEIGHTS")
...
mkdir -p "$OUTPUT_DIR"
LOG_FILE="$OUTPUT_DIR/run_flagship_$(date +%Y%m%d_%H%M%S).log"

# === Execution ===
echo "=== Starting Full Fine-tuning Experiment (FLAGSHIP - BCIC2A) ===" | tee -a "$LOG_FILE"
echo "Timestamp: $(date)" | tee -a "$LOG_FILE"
echo "Output Directory: $OUTPUT_DIR" | tee -a "$LOG_FILE"

export CUDA_VISIBLE_DEVICES="$GPU_IDS"

python experiments/bcic2a_full_ft/run_full_ft_compare.py \
    --config "$CONFIG" \
