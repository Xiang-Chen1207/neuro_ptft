...
OUTPUT_DIR="/vePFS-0x0d/home/cx/ptft/output/joint_pretrain"
mkdir -p $OUTPUT_DIR

echo "Starting Joint Pretraining Experiment (TUEG, SEED, Workload_MATB, SleepEDF, SHU_MI)"
...
python3 main.py \
  ...
  --opts \
    ...
    project="ptft-joint-reconstruction" \
    epochs=50 \
    dataset.name="JOINT" \
