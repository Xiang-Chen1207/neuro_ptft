...
OUTPUT_DIR="/vePFS-0x0d/home/cx/ptft/output/pretrain_st_eegformer_large"
mkdir -p $OUTPUT_DIR

echo "Starting ST-EEGFormer (Small) Pretraining Experiment on GPU 0,1"
...
# Start Training
python3 main.py \
  --config configs/pretrain_st_eegformer.yaml \
  $RESUME_ARG \
  --opts \
    output_dir=$OUTPUT_DIR \
    enable_wandb=$ENABLE_WANDB \
    entity="cx2521-new-york-university" \
    project="ptft-st-eegformer-large" \
    epochs=50 \
    dataset.name="JOINT" \
