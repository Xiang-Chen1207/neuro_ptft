...
OUTPUT_DIR=${OUTPUT_DIR:-"output/tueg_dev/csbrain_feat_only_10s"}
PROJECT_NAME=${PROJECT_NAME:-"ptft-csbrain-feat-only-10s"}

mkdir -p "$OUTPUT_DIR" "$SHARED_CACHE_DIR"
RESUME_ARG=""
CHECKPOINT_PATH="${OUTPUT_DIR}/latest.pth"
if [ "$1" == "resume" ] && [ -f "$CHECKPOINT_PATH" ]; then
  RESUME_ARG="--resume $CHECKPOINT_PATH"
elif [ -f "$CHECKPOINT_PATH" ]; then
  RESUME_ARG="--resume $CHECKPOINT_PATH"
fi

python3 main.py --config configs/pretrain_csbrain_tueg_10s.yaml $RESUME_ARG --opts \
  output_dir="$OUTPUT_DIR" \
  project="$PROJECT_NAME" \
  enable_wandb=$ENABLE_WANDB \
  dataset.batch_size=$BATCH_SIZE \
  dataset.num_workers=$NUM_WORKERS \
  dataset.train_num_workers=$NUM_WORKERS \
  ...
  dataset.input_size=1000 \
  dataset.required_num_channels=21 \
