...
def get_bcic2a_file_list(dataset_dir, mode='train', seed=42):
    """
    Get file list for BCIC2A dataset.
    Splitting: First 7 subjects for train, last 2 for test (total 9 subjects).
    """
    ...
    # Sort by subject ID to ensure consistent order (sub_1, sub_2, ... sub_9)
    ...
    if len(all_h5_files) < 9:
        print(f"Warning: Expected 9 subjects for BCIC2A, found {len(all_h5_files)}")
...
class BCIC2ADataset(Dataset):
    def __init__(self, file_list, input_size=1000, transform=None, cache_path='dataset_index_bcic2a.json', mode='train', **kwargs):
        super().__init__()
        self.input_size = input_size # Default to 4s (1000 samples @ 250Hz)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Read all
            raw = dset[:] # Shape (22, 1000)
            
            # Ensure shape is (Channels, Time)
            if raw.shape[0] != 22 and raw.shape[1] == 22:
                raw = raw.T
